...
(function (Drupal) {
  ...
  function isDesktopNav() {
    var navButtons = document.querySelector(
  ...
  Drupal.katie.isDesktopNav = isDesktopNav;
  var stickyHeaderToggleButton = document.querySelector(
    '[data-drupal-selector="sticky-header-toggle"]'
  );
  var siteHeaderFixable = document.querySelector(
  ...
  function setStickyHeaderStorage(expandedState) {
    ...
    localStorage.setItem(
      "Drupal.katie.stickyHeaderState",
      JSON.stringify(item)
    );
  }
  ...
  function getStickyHeaderStorage() {
    var stickyHeaderState = localStorage.getItem(
      "Drupal.katie.stickyHeaderState"
    );
    ...
    if (now.getTime() > item.expiry) {
      localStorage.removeItem("Drupal.katie.stickyHeaderState");
      return false;
    }
    return item.value;
  }
  if (
    ...
    var fixableElements = document.querySelectorAll(
    ...
    var siteHeaderInner = document.querySelector(
    ...
    if (siteHeaderInner) {
      siteHeaderInner.addEventListener("focusin", function () {
        if (isDesktopNav() && !stickyHeaderIsEnabled()) {
          ...
          var headerNav = header.querySelector(
...
(function (Drupal) {
  ...
  Drupal.katie.searchIsVisible = searchIsVisible;
  ...
  function watchForFocusOut(e) {
    if (e.relatedTarget) {
      var inSearchBar = e.relatedTarget.matches(
        ""
          .concat(searchWideWrapperSelector, ", ")
          .concat(searchWideWrapperSelector, " *")
      );
      var inSearchButton = e.relatedTarget.matches(
        ""
          .concat(searchWideButtonSelector, ", ")
          .concat(searchWideButtonSelector, " *")
      );
  ...
  Drupal.katie.toggleSearchVisibility = toggleSearchVisibility;
  Drupal.behaviors.searchWide = {
    attach: function attach(context) {
      var searchWideButtonEl = once(
        "search-wide",
...
(function (Drupal, once, tabbable) {
  ...
  function init(props) {
    ...
    document.addEventListener("keyup", function (e) {
      if (e.key === "Escape" || e.key === "Esc") {
        if (props.katie.areAnySubNavsOpen()) {
          props.katie.closeAllSubNav();
        } else {
  ...
  Drupal.behaviors.katieNavigation = {
    attach: function attach(context) {
      var headerId = "header";
      ...
      if (header) {
        ...
        var katie = Drupal.katie;
        var navButton = context.querySelector(
          '[data-drupal-selector="mobile-nav-button"]'
        );
        var body = document.body;
        var overlay = context.querySelector(
 ...
 **/
(function (Drupal) {
  var isDesktopNav = Drupal.katie.isDesktopNav;
  var secondLevelNavMenus = document.querySelectorAll(
  ...
  Drupal.katie.toggleSubNav = toggleSubNav;
  function handleBlur(e) {
    if (!Drupal.katie.isDesktopNav()) return;
    setTimeout(function () {
      var menuParentItem = e.target.closest(
        '[data-drupal-selector="primary-nav-menu-item-has-children"]'
      );
  ...
  Drupal.katie.closeAllSubNav = closeAllSubNav;
  ...
  Drupal.katie.areAnySubNavsOpen = areAnySubNavsOpen;
  document.addEventListener("keyup", function (e) {
    if (e.key === "Escape" || e.key === "Esc") {
...
(function (Drupal, once) {
  ...
  function checkIfDesktopNavigationWraps(entries) {
    ...
    if (
      Drupal.katie.isDesktopNav() &&
      entries[0].contentRect.height > navItem.clientHeight
    ) {
      var navMediaQuery = window.matchMedia(
        "(max-width: ".concat(window.innerWidth + 5, "px)")
      );
      document.body.classList.add("is-always-mobile-nav");
